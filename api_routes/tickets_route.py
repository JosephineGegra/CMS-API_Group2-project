...
@ticket_route.post("/", response_model=Schema.TicketResponse)
def create_ticket(ticket: Schema.TicketCreate, db: Session = Depends(get_db)):
    return ticket_service.create_ticket(db, ticket)

@ticket_route.get("/", response_model=list[Schema.TicketResponse])
def get_all_tickets(db: Session = Depends(get_db),
    current_user: Models.User = Depends(get_current_user)
):
    return ticket_service.get_all_tickets(db)

@ticket_route.get("/{ticket_id}", response_model=Schema.TicketResponse)
...
@ticket_route.put("/{ticket_id}", response_model=Schema.TicketResponse)
